...
//    }
        ...
        public List<List<String>> avail(List < List < String >> cal1, List < List < String >> cal2, List < String > rang1, List < String > rang2, int duration){
            //O(1)
            List<List<String>> unavailTimes = new ArrayList<>();
            List<List<String>> potentialMeetingTimes = new ArrayList<>();
            String previousStart = "";
            String previousEnd = "";
            int i = 0;
            int j = 0;
            ...
            while (i < cal1.size() && j < cal2.size()) {
                ...
                if (!unavailSlot.start.equals(previousStart) && !unavailSlot.end.equals(previousEnd)) {
                    unavailTimes.add(unavailSlot.getList());
            ...
            //O(n)
            for (int j2 = 0; j2 < unavailTimes.size() - 1; j2++) {
                String currentEnd = unavailTimes.get(j2).get(1);
                String nextStart = unavailTimes.get(j2 + 1).get(0);
                StringMeeting availSlot = new StringMeeting(currentEnd, nextStart);
                if (withinRange(availSlot, availSlot, availSlot.start, availSlot.end, duration)) {
                    potentialMeetingTimes.add(availSlot.getList());
                }
            }
        ...
        public static boolean withinRange (StringMeeting rang1, StringMeeting rang2, String stime, String etime,
        int duration){
