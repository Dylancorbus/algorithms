...
public class Practice {
    ...
    public static void main(String[] args) {
        try {
            ...
            lines.stream()
                    .map(str -> new Amazon.LogLine(str))
                    .collect(groupingBy(Amazon.LogLine::getId, mapping(Amazon.LogLine::getPath, toList())))
                    .entrySet()
                    .stream()
...
//            String map3 = map.entrySet()
//                    .stream()
//                    .filter(stringListEntry -> !stringListEntry.getKey().equals(map2))
//                    .reduce((o1, o2) -> o2)
//                    .map(entry -> entry.getKey() + " " + entry.getValue().stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
//                            .entrySet()
//                            .stream()
//                            .sorted( (e1, e2) -> e1.getValue() > e2.getValue() ? -1 : 1)
//                            .map(mE -> mE.getKey())
...
//                    .orElse("Not Found");
//            String map4 = map.entrySet()
//                    .stream()
//                    .filter(stringListEntry -> !stringListEntry.getKey().equals(map3.split(" ")[0]) && !stringListEntry.getKey().equals(map2))
//                    .map(entry -> entry.getKey() + " " + entry.getValue().stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
//                            .entrySet()
//                            .stream()
//                            .sorted( (e1, e2) -> e1.getValue() > e2.getValue() ? -1 : 1)
